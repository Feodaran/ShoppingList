...
namespace ShoppingList
{
    public partial class FormShoppingList : Form
    {
        ...
        private void btnAddItem_Click(object sender, EventArgs e)
        {
            ...
            double result = Convert.ToDouble(itemPrice * quantity);

            txtShoppingList.Text = Convert.ToString(result);

            //txtShoppingList.AppendText(result);



            //itemPrice = Convert.ToDouble(txtPrice.Text);
            //quantity = Convert.ToInt32(txtQuantity.Text);
            //lblTotalPrice.Text = Convert.ToString(itemPrice * quantity);
